...
class ExcelController(Controller):
    ...
    def _process_links(self, message, links_per_profile=3):
        ...
        try:
            ...
            # Continue processing until all links are done
            while remaining_links and len(suspended_profiles) < len(profile_ids):
                ...
                for i, profile_id in enumerate(profile_ids):
                            ...
                            try:
                                ...
                                if result['status'] == 'permanent_skip':
                                    if result['reason'] == 'Profile not logged in':
                                        ...
                                        # Put unprocessed links back in queue
                                        remaining_links = current_batch[index-1:] + remaining_links
                                        break
                    ...
                    if remaining_links and i < len(profile_ids) - 1:  # Don't sleep after last profile or if no links left
                        logging.info("Waiting 15 seconds before next profile...")
                        time.sleep(15)
                
                # If we still have links but went through all profiles, log the restart
                if remaining_links and len(suspended_profiles) < len(profile_ids):
                    logging.info("Still have links to process, starting new round with available profiles...")
